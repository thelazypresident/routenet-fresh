...
import { formatMoney, formatDate as formatDateUtil } from '../utils/formatters';
import { toast } from 'sonner';
import { format } from 'date-fns';

export default function ExportTransactionsModal({ open, onClose, transactions, defaultStartDate = '', defaultEndDate = '' }) {
  ...
  // Reset custom date range when modal opens
  React.useEffect(() => {
    if (open) {
  ...
  const filterTransactions = () => {
    ...
    // If user set custom date range, use that
    if (dateRange.start || dateRange.end) {
      ...
      // Otherwise use the page's current date filter
      if (defaultStartDate) {
        filtered = filtered.filter(t => t.date >= defaultStartDate);
      }
      if (defaultEndDate) {
        filtered = filtered.filter(t => t.date <= defaultEndDate);
      }
    }
  ...
  // Get the effective date range for display
  ...
  // Check if both dates are set (either custom or default)
  const hasCustomDates = dateRange.start && dateRange.end;
  const hasDefaultDates = defaultStartDate && defaultEndDate;
  const canExport = hasCustomDates || hasDefaultDates;
  ...
  const exportToPDF = () => {
    ...
    if (effectiveRange.start && effectiveRange.end) {
      if (effectiveRange.start === effectiveRange.end) {
        dateRangeText = format(new Date(effectiveRange.start), 'MMM dd, yyyy');
      } else {
        dateRangeText = `${format(new Date(effectiveRange.start), 'MMM dd, yyyy')} – ${format(new Date(effectiveRange.end), 'MMM dd, yyyy')}`;
      }
    } else if (effectiveRange.start) {
      dateRangeText = `From ${format(new Date(effectiveRange.start), 'MMM dd, yyyy')}`;
    } else if (effectiveRange.end) {
      dateRangeText = `Until ${format(new Date(effectiveRange.end), 'MMM dd, yyyy')}`;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={`max-w-md ${isDark ? 'card-dark text-white' : 'bg-white'} [&>button]:hidden`}>
        <DialogHeader>
        ...
        <div className="space-y-4">
          <div>
            <Label className="mb-2 block">Date Range (Override)</Label>
            <p className={`text-xs mb-2 ${isDark ? 'text-white/50' : 'text-gray-500'}`}>
              {defaultStartDate && defaultEndDate ? (
                defaultStartDate === defaultEndDate 
                  ? `Leave empty to use current filter: ${format(new Date(defaultStartDate), 'MMM dd, yyyy')}`
                  : `Leave empty to use current filter: ${format(new Date(defaultStartDate), 'MMM dd')} – ${format(new Date(defaultEndDate), 'MMM dd')}`
              ) : (
          ...
          <div className="space-y-2">
            ...
            <Button 
              ...
              disabled={!canExport}
              variant="outline"
              className={`w-full ${isDark ? 'btn-secondary' : ''} disabled:opacity-40 disabled:cursor-not-allowed`}
            >
